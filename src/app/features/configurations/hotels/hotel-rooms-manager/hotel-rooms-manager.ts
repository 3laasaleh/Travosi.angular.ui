...
@Component({
  ...
  standalone: true,
  imports: [FormsModule, ReactiveFormsModule, TranslatePipe, HotelRoomCard],
...
export class HotelRoomsManager implements OnChanges {
  ...
  readonly roomForm = new FormGroup({
    ...
    roomSize: new FormControl<number | null>(null), sizeUnit: new FormControl('sqm', { nonNullable: true }),
    bedConfiguration: new FormControl('', { nonNullable: true }), numberOfBathrooms: new FormControl(1, { nonNullable: true, validators: [Validators.min(0)] }),
    ...
    totalInventory: new FormControl(0, { nonNullable: true, validators: [Validators.min(0)] }), costPrice: new FormControl(0, { nonNullable: true, validators: [Validators.min(0)] }),
    sellingPrice: new FormControl(0, { nonNullable: true, validators: [Validators.min(0)] }), isPublished: new FormControl(false, { nonNullable: true }),
  });
  ...
  reset(): void {
    ...
    this.roomImages = [];
    this.imageMessage = '';
    this.editingRoom = null; this.error = ''; this.selectedFacilityIds = new Set();
    this.roomForm.reset({ nameEng: '', nameAr: '', descriptionEng: '', descriptionAr: '', roomType: 1, mealPlan: 1, roomSize: null, sizeUnit: 'sqm', bedConfiguration: '', numberOfBathrooms: 1, smokingAllowed: false, maxAdults: 1, maxChildren: 0, maxInfants: 0, totalInventory: 0, costPrice: 0, sellingPrice: 0, isPublished: false });
    ...
    this.rates.clear(); this.policies.clear();
  }
  ...
  addPeriod(value: any = {}): void { this.rates.push(new FormGroup({ startDate: new FormControl(value.startDate ?? '', { nonNullable: true, validators: [Validators.required] }), endDate: new FormControl(value.endDate ?? '', { nonNullable: true, validators: [Validators.required] }), nightlyRate: new FormControl(Number(value.nightlyRate ?? 0), { nonNullable: true, validators: [Validators.required, Validators.min(0.01)] }), isActive: new FormControl(value.isActive !== false, { nonNullable: true }) })); }
  ...
  save(): void {
    ...
    if (this.roomForm.invalid || this.roomForm.controls.sellingPrice.value < this.roomForm.controls.costPrice.value) { this.roomForm.markAllAsTouched(); this.error = this.roomForm.controls.sellingPrice.value < this.roomForm.controls.costPrice.value ? 'roomPriceRangeInvalid' : 'pleaseCorrectFormErrors'; return; }
    ...
    if (Number(rateValue.baseNightlyRate) <= 0 && Number(value.sellingPrice) > 0) this.rateForm.controls.baseNightlyRate.setValue(Number(value.sellingPrice));
    ...
    const payload = { hotelId: this.hotelId, name: value.nameEng.trim(), ...value, roomType: Number(value.roomType), mealPlan: Number(value.mealPlan), roomSize: value.roomSize === null ? null : Number(value.roomSize), numberOfBathrooms: Number(value.numberOfBathrooms), maxAdults: Number(value.maxAdults), maxChildren: Number(value.maxChildren), maxInfants: Number(value.maxInfants), maxTotalOccupancy: Number(value.maxAdults) + Number(value.maxChildren) + Number(value.maxInfants), totalInventory: Number(value.totalInventory), costPrice: Number(value.costPrice), sellingPrice: Number(value.sellingPrice), ...(this.editingRoom ? { id: Number(this.editingRoom.id) } : {}) };
  ...
  private validatePeriods(): boolean {
    ...
    return periods.every((period) => period.startDate && period.endDate && period.endDate > period.startDate && Number(period.nightlyRate) > 0) && periods.every((period, index) => index === 0 || periods[index - 1].endDate <= period.startDate);
  }

...
export class HotelFacilitiesPage implements OnInit {
  ...
  loadCatalog(): void {
    forkJoin({
      ...
      if (!this.form.controls.facilityCategoryId.value && this.categories[0])
        this.form.controls.facilityCategoryId.setValue(this.categories[0].id);
  ...
  load(): void {
    ...
    this.api
      ...
      .pipe(
        catchError(() => {
          this.error = 'hotelFacilitiesLoadError';
          return of(null);
        }),
        finalize(() => {
          this.loading = false;
  ...
  applyFilter(): void {
    ...
    this.filtered = !value
      ? this.facilities
      : this.facilities.filter((x) =>
          `${x.nameEng} ${x.nameAr} ${x.iconKey}`.toLowerCase().includes(value),
        );
  }
  ...
  save(): void {
    ...
    request
      .pipe(
        catchError(() => {
          this.error = 'hotelFacilitiesSaveError';
          return of(null);
        }),
        finalize(() => {
          this.saving = false;
  ...
  categoryName(value: number): string {

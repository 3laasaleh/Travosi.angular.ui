...
@Component({ selector: 'app-blog-detail', standalone: true, imports: [RouterLink, DatePipe, TranslatePipe, HomeNavbar, FooterOne, ImageViewerModal, DescriptionLinks], templateUrl: './blog-detail.html', changeDetection: ChangeDetectionStrategy.OnPush })
export class BlogDetail implements OnInit {
  ...
  ngOnInit(): void { const routeName = this.route.snapshot.paramMap.get('routeName')?.trim(); if (!routeName) { this.errorMessage = 'Blog not found.'; this.isLoading = false; this.seo.markNotFound('Blog not found'); return; }
    this.api.getUnauthntecated(`Blogs/by-route/${encodeURIComponent(routeName)}`).pipe(catchError(() => { this.errorMessage = 'Blog not found.'; return of(null); }), finalize(() => { this.isLoading = false; this.cdr.markForCheck(); })).subscribe(response => { if (response?.isSuccess === false || !response) { this.errorMessage = response?.message || 'Blog not found.'; this.seo.markNotFound('Blog not found'); return; } this.blog = response.data ?? response; this.selectedImageIndex = 0; this.syncImageGallery(); this.seo.updateFrom(this.blog, { image: this.imageSources[0], imageUrl: this.images[0], schemaType: 'BlogPosting' }); }); }
  ...
  get content(): string { return this.headerData.length ? '' : this.localizedRawContent; }
  get headerData(): Array<{ headerType: number; header: string; description: string }> {
    ...
    if (Array.isArray(sections) && sections.length) {
      return sections
        .slice(0, 5)
        .sort((left: any, right: any) => Number(left?.orderNumber ?? left?.OrderNumber) - Number(right?.orderNumber ?? right?.OrderNumber))
        .map((section: any) => ({
          ...
          header: this.isArabic
            ...
            : (section?.headerEng ?? section?.HeaderEng ?? section?.header ?? section?.Header ?? ''),
          description: this.isArabic
            ? (section?.descriptionAr ?? section?.DescriptionAr ?? section?.description ?? section?.Description ?? '')
            : (section?.descriptionEng ?? section?.DescriptionEng ?? section?.description ?? section?.Description ?? ''),
        }));
    }
  ...
  private get localizedRawContent(): string { return this.blog?.content ?? (this.isArabic ? (this.blog?.contentAr || this.blog?.contentEng || '') : (this.blog?.contentEng || this.blog?.contentAr || '')); }
  private parseLegacyHeaderData(content: string): Array<{ headerType: number; header: string; description: string }> {
    if (!content?.trim()) return [];
    try {
      const parsed = JSON.parse(content);
      return Array.isArray(parsed)
        ? parsed.slice(0, 5).filter((item) => item && typeof item === 'object').map((item) => ({

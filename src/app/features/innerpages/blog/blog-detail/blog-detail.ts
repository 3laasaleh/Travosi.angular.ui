...
export class BlogDetail implements OnInit {
  ...
  ngOnInit(): void { const id = Number(this.route.snapshot.paramMap.get('id')); if (!Number.isInteger(id) || id <= 0) { this.errorMessage = 'Blog not found.'; this.isLoading = false; return; }
    this.api.getUnauthntecated(`Blogs/${id}`).pipe(catchError(() => { this.errorMessage = 'Blog not found.'; return of(null); }), finalize(() => { this.isLoading = false; this.cdr.markForCheck(); })).subscribe(response => { if (response?.isSuccess === false || !response) { this.errorMessage = response?.message || 'Blog not found.'; return; } this.blog = response.data ?? response; this.selectedImageIndex = 0; this.syncImageGallery(); }); }
  ...
  get images(): string[] { return Array.isArray(this.blog?.images) ? this.blog.images.map((image: any) => this.image(image)) : []; }

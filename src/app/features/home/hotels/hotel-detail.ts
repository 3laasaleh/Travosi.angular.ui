...
@Component({
  ...
  standalone: true,
  imports: [Breadcrumbs, DatePicker, FooterOne, FormsModule, HomeNavbar, TranslatePipe, HotelRoomCard, ImageViewerModal, ProductReviews],
...
export class HotelDetail implements OnInit {
  ...
  get hasSearchedAvailability(): boolean { return this.availabilitySearched && !this.searching; }
  get availableRooms(): any[] {
    return (this.hotel?.rooms ?? []).filter((room: any) => (this.roomAvailability(room.id)?.availableQuantity ?? 0) >= this.roomCount);
  }
  ...
  get googleMapsUrl(): string | null {
    ...
    return /^https:\/\/(?:www\.)?google\.[^/]+\/maps|^https:\/\/maps\.app\.goo\.gl\//i.test(url) ? url : null;
  }

  ngOnInit(): void {
    this.applySearchQuery();
    this.route.paramMap.pipe(
      switchMap((params) => this.api.getUnauthntecated(`Hotels/Public/${encodeURIComponent(params.get('routeName') ?? '')}`).pipe(catchError(() => of(null)))),
      finalize(() => { this.loading = false; this.cdr.markForCheck(); }),
    ).subscribe((response: any) => {
      this.hotel = response?.data ?? null;
      if (!this.hotel) return;
      this.seo.updateFrom(this.hotel, { image: this.images[0], imageUrl: this.resolvedImages[0], schemaType: 'Place' });
  ...
  closeGuestMenuOnOutsideClick(event: MouseEvent): void {
    if (!this.guestControl?.nativeElement.contains(event.target as Node)) this.guestMenuOpen = false;
  }

  searchAvailability(): void {
    this.validationSubmitted = true;
    this.availabilityError = '';
    if (!this.searchValuesValid()) { this.availabilitySearched = false; return; }
    const query = new URLSearchParams({
      hotelId: String(this.hotel.id), checkInDate: this.checkInDate, checkOutDate: this.checkOutDate,
      adults: String(this.adults), children: String(this.children), infants: String(this.infants), roomCount: String(this.roomCount),
    });
    ...
    this.searching = true;
    this.api.getUnauthntecated(`HotelAvailability?${query.toString()}`).pipe(
      catchError((error) => { this.availabilityError = error?.error?.message || 'hotelAvailabilitySearchFailed'; return of(null); }),
      finalize(() => { this.searching = false; this.cdr.markForCheck(); }),
    ).subscribe((response: any) => {
      if (!response || response?.isSuccess === false) {
        this.availabilityError = response?.message || this.availabilityError || 'hotelAvailabilitySearchFailed';
        return;
      }
  ...
  async reserveRoom(room: any): Promise<void> {
    ...
    const confirmation = await Swal.fire({
      ...
      icon: 'question', showCancelButton: true, confirmButtonText: this.translate.instant('confirm'),
      cancelButtonText: this.translate.instant('cancel'), confirmButtonColor: '#0891b2',
    });
    if (!confirmation.isConfirmed) return;
    this.bookingRoomId = Number(room.id);
    this.cdr.markForCheck();
    this.api.post('Bookings', {
      hotelId: this.hotel.id, hotelRoomId: room.id, roomCount: this.roomCount,
      adults: this.adults, children: this.children, childrenAges: this.childrenAges, infants: this.infants,
      numberOfTravelers: this.travelers, dateFrom: this.checkInDate, dateTo: this.checkOutDate,
    }).pipe(finalize(() => { this.bookingRoomId = null; this.cdr.markForCheck(); })).subscribe({
      next: async (response: any) => {
        if (!response?.isSuccess) {
          await Swal.fire({ icon: 'error', text: this.translate.instant(response?.message || 'hotelRoomUnavailable') });
          return;
        }
        await Swal.fire({
          icon: 'success', title: this.translate.instant('hotelBookingRequest'),
          text: this.translate.instant(response?.data?.acknowledgementEmailSent === false ? 'bookingConfirmationEmailPending' : 'hotelBookingConfirmationEmailSent'),
        });
  ...
  imageAlt(image: any): string { return this.utility.imageAlt(image, this.hotelName()); }
  onImageError(event: Event): void { this.utility.onImageError(event); }
  childPolicyText(policy: any): string {
    return this.localized(policy?.valueEng ?? policy?.descriptionEng, policy?.valueAr ?? policy?.descriptionAr);
  }
  hotelName(): string { return this.localized(this.hotel?.nameEng ?? this.hotel?.name, this.hotel?.nameAr); }
  hotelDescription(): string {
    return this.localized(this.hotel?.descriptionEng ?? this.hotel?.description, this.hotel?.descriptionAr ?? this.hotel?.descriptionAR);
  }
  hotelAddress(): string {
    return this.localized(this.hotel?.addressEng ?? this.hotel?.address ?? this.hotel?.destinationName, this.hotel?.addressAr ?? this.hotel?.destinationName);
  }
  ...
  private searchValuesValid(): boolean {
    return !!this.hotel?.id && !!this.checkInDate && !!this.checkOutDate && this.checkInDate >= this.today && this.checkOutDate > this.checkInDate
      && Number.isInteger(Number(this.roomCount)) && Number(this.roomCount) >= 1
      && Number.isInteger(Number(this.adults)) && Number(this.adults) >= 1
      && Number.isInteger(Number(this.children)) && Number(this.children) >= 0
      && Number.isInteger(Number(this.infants)) && Number(this.infants) >= 0

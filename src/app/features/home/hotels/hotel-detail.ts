...
export class HotelDetail implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap
      .pipe(
        switchMap((p) =>
          this.api
            .getUnauthntecated(`Hotels/Public/${encodeURIComponent(p.get('routeName') ?? '')}`)
      ...
      .subscribe((r: any) => {
        this.hotel = r?.data ?? null;
        if (this.hotel) {
          this.seo.updateFrom(this.hotel, {
            image: this.hotel.images?.find((image: any) => image.isMain) ?? this.hotel.images?.[0],
            schemaType: 'Place',
          });
  ...
  get hasSearchedAvailability(): boolean {
    return !!this.checkInDate && !!this.checkOutDate && !this.searching;
  }
  ...
  async reserveRoom(room: any): Promise<void> {
    ...
    const confirmation = await Swal.fire({
      ...
      text: `${room?.name ?? ''} — ${price.grandTotal ?? price.total ?? 0} USD`,
      icon: 'question',
      ...
      .subscribe({
        next: async (response: any) => {
          ...
          await Swal.fire({
            icon: 'success',
            title: this.translate.instant('hotelBookingRequest'),
            text: this.translate.instant('hotelBookingConfirmation'),
          });
  ...
  childPolicyText(policy: any): string {
    return this.localized(
      policy?.descriptionEng || `Ages ${policy?.minAge}–${policy?.maxAge}`,
      policy?.descriptionAr || `الأعمار ${policy?.minAge}–${policy?.maxAge}`,
    );
  }
  hotelName(): string {
  ...
  hotelAddress(): string {
    return this.localized(this.hotel?.addressEng ?? this.hotel?.address, this.hotel?.addressAr);
  }

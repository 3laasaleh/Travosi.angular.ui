...
export class Destinations implements OnInit {
  ...
  loadDestinations(): void {
    ...
    this.adminService.getDestinations(1, 100).pipe(
      catchError(() => {
        this.errorMessage = 'The destination service is currently unavailable. Showing local sample data.';
  ...
  deactivateDestination(destination: any): void {
    this.isLoading = true;
    this.adminService.deactivateDestination(destination.id).pipe(catchError(() => {
  ...
  private getFallbackDestinations(): any[] {
    return [
      { id: 1, name: 'Santorini', country: 'Greece', city: 'Cyclades', description: 'A classic island escape.', imageUrl: 'https://images.unsplash.com/photo-1570077188670-e3a8d69ac5ff', isActive: true },
      { id: 2, name: 'Marrakech', country: 'Morocco', city: 'Marrakech', description: 'A vibrant desert and culture destination.', imageUrl: 'https://images.unsplash.com/photo-1548013146-72479768bada', isActive: true },
      { id: 3, name: 'Reykjavík', country: 'Iceland', city: 'Reykjavík', description: 'Northern lights and volcanic landscapes.', imageUrl: 'https://images.unsplash.com/photo-1500530855697-b586d89ba3ee', isActive: false },
    ];
  }
}
